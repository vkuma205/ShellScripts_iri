...
from pos_tx_fact f
  ...
  left join (select transaction_date,max(pointsearned) points_awarded ,max(pointsredeemed) points_redeemed,transactionnumber,retailer_product_identifier  from pos_txpoint_fact where tm_dim_key_day=${hiveconf:time_key} group by transactionnumber,retailer_product_identifier,transaction_date) p on f.trans_number=p.transactionnumber and f.retailer_product_identifier=p.retailer_product_identifier
left join saveon_retail_metric m on f.retailer_product_identifier=m.retailer_product_identifier  
where f.tm_dim_key_day  =${hiveconf:time_key} and custom_col3 in ('0', '4', '5', '9') and f.retailer_product_identifier not in ('220', '45', '101', '45636401848', '115', '100', '102', '190')
...
truncate table of_src_stg_tmp;
insert overwrite table of_src_stg_tmp partition(enddatekey)  
select * from of_src_stg where enddatekey <= ${hiveconf:weekendkey} and startdatekey>=${hiveconf:weekstartkey};


insert overwrite table causal partition(tm_dim_key_day)
...
from(
select f.*,NVL(b.BASKET_ATTR_DIM_KEY,1) BASKET_ATTR_DIM_KEY,nvl(o.of_dim_key,970472) of_dim_key,row_number() over(partition by f.tm_dim_key_day,f.trans_number,f.retailer_product_identifier,f.item_gross_amount order by case when o.of_dim_key is null then 100 else sm.group_rank end) as rn
from pos_tx_agg  f
left join store_mapping sm on f.venue_number=sm.locationid
left join of_src_stg_tmp  o on o.retailer_product_identifier=f.retailer_product_identifier and o.locationid=sm.master_location_id
...
where f.tm_dim_key_day =${hiveconf:time_key} and ((item_gross_amount<>0 and item_quantity<>0) or shrink_dollars <> 0)
) x where rn=1
distribute by tm_dim_key_day;

insert overwrite table ecomm_sales PARTITION (tm_dim_key_day) select distinct trans_number,tm_dim_key_day from pos_tx_fact where tm_dim_key_day =${hiveconf:time_key};
